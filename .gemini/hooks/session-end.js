#!/usr/bin/env node
/**
 * LMAS Session End Hook for Gemini CLI
 * Story GEMINI-INT.6 - LMAS Hooks System
 *
...
async function sessionEnd() {
  ...
  try {
    const sessionDir = path.join(projectDir, '.lmas', 'sessions');
    ...
    let modifiedFiles = [];
    const sessionFilesPath = path.join(projectDir, '.lmas', 'session-files.json');
    ...
    let toolCount = 0;
    const toolLogPath = path.join(projectDir, '.lmas', 'logs', 'tool-results.jsonl');

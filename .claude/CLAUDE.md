...
O LMAS suporta multiplos projetos no mesmo vault. Cada projeto tem sua propria pasta em `projects/`.

### Estrutura

```
projects/
  lmas/          # Framework LMAS (codigo neste dir)
  clawin/        # ClaWin1Click (codigo em C:\Users\luanf\openclaw)
  i5x/           # i5x WhatsApp (codigo em C:\Users\luanf\Documents\dash_i5x)
```

Cada projeto contem: `PROJECT-CHECKPOINT.md`, `CLAUDE.md`, `.project.yaml`, `stories/`, `prd/`, `architecture/`.

### Como funciona

1. O hook `checkpoint-context.cjs` lista todos os projetos no inicio da sessao
2. O agente pergunta ao usuario qual projeto (se nao mencionado)
3. O projeto ativo e armazenado no **contexto da conversa** (NAO em arquivo)
4. Todas as operacoes (checkpoint, stories, context-load) usam `projects/{id}/`
5. Duas sessoes simultaneas podem trabalhar em projetos diferentes sem conflito

### Story ID Convention

- LMAS: `LMAS-{epic}.{story}`
- ClaWin: `CW-{epic}.{story}`
- i5x: `I5X-{epic}.{story}`

### Regras para agentes

- **Context-Load** (Rule 1): Resolver projeto ativo ANTES de ler docs
- **Checkpoint**: Escrever em `projects/{id}/PROJECT-CHECKPOINT.md`
- **Handoff**: Incluir `project_id` no artefato
- **Daily Notes**: Usar formato `[@agent][projeto]` em multi-project
- **Code Path**: Ler `projects/{id}/.project.yaml` → `code_path` para saber onde o codigo vive
...
| Agente | Persona | Domínio | Escopo Principal |
|--------|---------|---------|------------------|
| `@dev` | Neo | software-dev | Implementação de código |
| `@qa` | Oracle | software-dev | Testes e qualidade |
| `@architect` | Aria | software-dev | Arquitetura e design técnico |
...
| `@analyst` | Atlas | software-dev | Pesquisa e análise |
...
| `@marketing-chief` | Lock | marketing | Aprovação de conteúdo, brand governance |
| `@copywriter` | Mouse | marketing | Copywriting para todos os canais |
| `@social-media-manager` | Sparks | marketing | Publicação (EXCLUSIVO), calendário social |
| `@content-strategist` | Persephone | marketing | Estratégia de conteúdo, editorial |
| `@content-researcher` | Ghost | marketing | Pesquisa de mercado, concorrentes |
| `@content-reviewer` | Seraph | marketing | Quality gate de conteúdo |
| `@seo` | Cypher | marketing | SEO audits, keywords, E-E-A-T, GEO |
| `@kamala` | Kamala | brand | Posicionamento, naming, identidade de marca |

...
## Session Continuity — Project Checkpoint (MUST)

**Protocolo completo:** `.claude/rules/checkpoint-protocol.md`

### No início de TODA sessão:
1. **Leia `docs/PROJECT-CHECKPOINT.md`** — estado completo do projeto
2. **Verifique "Próximos Passos"** para saber o que fazer a seguir

### Após QUALQUER ação relevante (automático, TODOS os agentes):
O agente que executa a ação DEVE atualizar `docs/PROJECT-CHECKPOINT.md` imediatamente:
- Story criada, completada, ou status mudou
- Documento criado ou atualizado (PRD, Architecture, ADR)
- Agente, workflow, ou task criada
- Subtask ou task concluída
- Decisão arquitetural tomada
- Push/merge realizado
- Sessão encerrando

**O agente que fez a ação atualiza.** Não delegue ao usuário. Não acumule — atualize imediatamente.
